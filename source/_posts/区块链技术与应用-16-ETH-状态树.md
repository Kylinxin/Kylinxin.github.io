---
title: "区块链技术与应用 16：ETH - 状态树（面试专题）"
...
> 面试一句话：以太坊状态树是全局账户状态的加密承诺。它用 Modified Merkle Patricia Trie 维护 `address -> account state`，每个区块执行完交易后都会得到新的 `stateRoot`，并写入区块头。

这一篇适合准备区块链开发、智能合约开发、节点工程和 Web3 后端岗位。面试官通常不是只问“状态树是什么”，而是继续追问为什么不用哈希表、为什么不用普通 Merkle Tree、状态树和合约存储有什么关系。
...
这个点在面试中很常见，因为它能区分 ETH 余额和 ERC-20 余额。
...
状态树是面试中的主线，交易树和收据树是它的对照组。下一篇会专门整理交易树和收据树。
...
面试回答时，不要只说“以太坊用 MPT”。更完整的回答是：以太坊需要维护动态全局账户状态，普通哈希表缺少证明能力，普通 Merkle Tree 不适合动态键值映射，所以使用 Modified MPT，在每个区块执行后得到新的 `stateRoot`。
